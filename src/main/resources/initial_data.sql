INSERT INTO PUBLIC.GENRES (ID, NAME)
VALUES (1, 'ДЕТСКАЯ');
INSERT INTO PUBLIC.GENRES (ID, NAME)
VALUES (2, 'САТИРА');
INSERT INTO PUBLIC.AUTHORS (ID, FULL_NAME)
VALUES (1, 'НИКОЛАЙ НОСОВ');
INSERT INTO PUBLIC.AUTHORS (ID, FULL_NAME)
VALUES (2, 'ИЛЬЯ ИЛЬФ');
INSERT INTO PUBLIC.AUTHORS (ID, FULL_NAME)
VALUES (3, 'ЕВГЕНИЙ ПЕТРОВ');
INSERT INTO PUBLIC.AUTHORS (ID, FULL_NAME)
VALUES (4, 'КАРНЕЙ ЧУКОВСКИЙ');
INSERT INTO PUBLIC.BOOKS
VALUES (1, 'ЗАТЕЙНИКИ', 1);
INSERT INTO PUBLIC.BOOKS
VALUES (2, 'ЗОЛОТОЙ ТЕЛЕНОК', 2);
INSERT INTO PUBLIC.BOOKS
VALUES (3, 'ТАРАКАНИЩЕ', 1);
INSERT INTO PUBLIC.BOOKS
VALUES (4, 'НЕЗНАЙКА НА ЛУНЕ', 1);
INSERT INTO PUBLIC.AUTHOR_TO_BOOK_MAP(AUTHOR_ID, BOOK_ID)
VALUES (1, 1);
INSERT INTO PUBLIC.AUTHOR_TO_BOOK_MAP(AUTHOR_ID, BOOK_ID)
VALUES (2, 2);
INSERT INTO PUBLIC.AUTHOR_TO_BOOK_MAP(AUTHOR_ID, BOOK_ID)
VALUES (3, 2);
INSERT INTO PUBLIC.AUTHOR_TO_BOOK_MAP(AUTHOR_ID, BOOK_ID)
VALUES (4, 3);
INSERT INTO PUBLIC.AUTHOR_TO_BOOK_MAP(AUTHOR_ID, BOOK_ID)
VALUES (4, 1);
INSERT INTO PUBLIC.book_comments (ID, BOOK_ID, COMMENT_TEXT)
VALUES ( 1, 1, 'ПОУЧИТЕЛЬНО');
INSERT INTO PUBLIC.book_comments (ID, BOOK_ID, COMMENT_TEXT)
VALUES ( 2, 1, 'ИНТЕРЕСНО');
INSERT INTO PUBLIC.book_comments (ID, BOOK_ID, COMMENT_TEXT)
VALUES ( 3, 2, 'АКТУАЛЬНО');